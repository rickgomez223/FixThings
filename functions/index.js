...
exports.handleFormSubmission = functions.https.onCall(async (data, context) => {
    const { name, email, phone, vin, vehicleType, year, make, model, timestamp } = data;
    ...
    try {
        ...
        if (ticketDoc.exists) {
            ...
            ticketNumber = 1; // Initialize if first time
        ...
        const pushcutUrl = 'https://api.pushcut.io/VEQktvCTFnpchKTT3TsIK/notifications/FWA'; // Replace with your Pushcut endpoint
        const pushcutData = {
            title: 'New Vehicle Submission',
            text: `Ticket #${ticketNumber} - Name: ${name}, Email: ${email}, Phone: ${phone}, VIN: ${vin || 'N/A'}, Type: ${vehicleType || 'N/A'}, Year: ${year || 'N/A'}, Make: ${make || 'N/A'}, Model: ${model || 'N/A'}`,
        };
